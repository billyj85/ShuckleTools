import asyncio

from levelupRoutes import create_pokestop_list, create_boost_xp_route, create_spawnpoint_route, fence, write_file, \
    create_xp_route, pokestop_ids
from mapelement_tools import load_map_elements, filter_map_elements

loop = asyncio.get_event_loop()


async def start():
    location_elements = load_map_elements("hamburg_source_data.txt")
    all_elements = fence("HamburgLeft").filter_forts(location_elements)
    xp_route_left = create_xp_route(all_elements, "hbg_left")
    write_file( "hamburg_xp1.py", "xp_route_1", str(xp_route_left))
    used_ids = pokestop_ids( xp_route_left)
    spawnpoint_route_left = create_spawnpoint_route(all_elements, used_ids, "spawnpoint_route_hl.gpx")
    write_file( "hamburg_grind1.py", "spawnpoint_route_1", str(spawnpoint_route_left))


loop.run_until_complete(start())
